...
EMPLOYEE_COLUMNS = ('Username', 'Name', 'Email', 'Phone', 'Branch')
EMPLOYEE_COLUMNS_SIZE = (25, 150, 60, 60, 60)
...
class ManagerApp:
    ...
    def __init__(self, master):
        ...
        self.master = master
        self.master.geometry(MANAGER_WINDOW_SIZE)
...
#                                  command=self.inventory, width=16)
        ...
        employees_button = tk.Button(self.frame, text='Employees', bg=my_config.FOREGROUND, command=self.show_employee, width=16)
        employees_button.grid(row=3, column=0, pady=(0, 3))
        
        customers_button = tk.Button(self.frame, text='Customers', bg=my_config.FOREGROUND, command=self.show_customers, width=16)
        customers_button.grid(row=4, column=0, pady=(0, 3))
        
        logoff_button = tk.Button(self.frame, text='Log Off', bg=my_config.FOREGROUND, command=self.log_off, width=16)
        logoff_button.grid(row=5, column=0, pady=(0, 3))
    ...
    def update_manager(self):
        ...
        """Allows manager to update his/her information"""
...
#        self.initialize_main_buttons()
        ...
        list_label = tk.Label(self.function_frame, text='Inventory',
        ...
        close_button = tk.Button(self.function_frame, text='Manager Menu',
                                 bg=my_config.FOREGROUND, command=self.initialize_main_buttons, width=16)
        close_button.grid(row=5, column=0)
...
#        self.initialize_main_buttons()
    ...
    def update_customer(self):
        ...
        """Allows manager to update customers"""
#        self.initialize_main_buttons()
    ...
    def show_employee(self):
        ...
        list_label = tk.Label(self.function_frame, text='Employees', width=100, bg=my_config.BACKGROUND)
        list_label.grid(row=0, column=0, pady=(10, 0))

        # creating treeview for customers
        self.employee_tree = Treeview(self.function_frame, columns=EMPLOYEE_COLUMNS, show='headings', height=10)
        self.employee_tree.grid(row=1, column=0, padx=8)

        for column_name, width in zip(EMPLOYEE_COLUMNS, EMPLOYEE_COLUMNS_SIZE):
            self.employee_tree.column(column_name, width=width, anchor=tk.CENTER)
            self.employee_tree.heading(column_name, text=column_name)

        # buttons
        update_employees_button = tk.Button(self.function_frame, text='Update Employee(s)',
                                       bg=my_config.FOREGROUND, command=self.update_employee, width=16)
        update_employees_button.grid(row=4, column=0)
        close_button = tk.Button(self.function_frame, text='Manager Menu',
                                       bg=my_config.FOREGROUND, command=self.initialize_main_buttons, width=16)
        close_button.grid(row=5, column=0)
    ...
    def update_employee(self):
        ...
        """Allows manager to update employees"""
    ...
    def log_off(self):
        """Returns Manager to login window."""
        if self.frame:
            self.frame.destroy()
        if self.function_frame:
            self.function_frame.destroy()
